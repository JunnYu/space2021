...
def formatter(_data, _meta):
    ...
    for _item in _data:
        ...
        if (
            _set_type == "dev" or _set_type == "val" or _set_type == "test"
        ) and _key not in _sheet:
            _sheet[_key] = {}
...
def subtask1_eval(_answers, _ref):
    ...
    _st["Accuracy"] = (_st["TP"] + _st["TN"]) / (
        _st["TP"] + _st["FP"] + _st["FN"] + _st["TN"]
    )
    return _st
...
def subtask3_eval(_answers, _ref):
    ...
    _st["Precision"] = (
        (_st["TP_2"] + _st["TN_2"]) / (_st["TN_1"] + _st["FN_1"])
        if (_st["TN_1"] + _st["FN_1"]) != 0
        else 0
    )
    _st["Recall"] = (
        (_st["TP_2"] + _st["TN_2"]) / (_st["TN_1"] + _st["FP_1"])
        if (_st["TN_1"] + _st["FP_1"]) != 0
        else 0
    )
    _st["F1"] = (
        2 * _st["Precision"] * _st["Recall"] / (_st["Precision"] + _st["Recall"])
        if (_st["Precision"] + _st["Recall"]) != 0
        else 0
    )
    return _st
...
if __name__ == "__main__":
    ...
    #
    my_path_1 = f"preds/task1-{dev_or_test}-with-answer.json"
    my_path_2 = f"preds/task2-{dev_or_test}-with-answer.json"
    my_path_3 = f"preds/task3-{dev_or_test}-with-answer.json"
    answers = []
    ...
    #
    ref_path_1 = f"dev/pack1.5/task1-{dev_or_test}-with-answer.json"
    ref_path_2 = f"dev/pack1.5/task2-{dev_or_test}-with-answer.json"
    ref_path_3 = f"dev/pack1.5/task3-{dev_or_test}-with-answer.json"
    answers = []
    ...
    that = evaluate(
        sheet,
        refs[f"subtask1-{dev_or_test}"],
        refs[f"subtask2-{dev_or_test}"],
        refs[f"subtask3-{dev_or_test}"],
    )
    print(that)
    pass
